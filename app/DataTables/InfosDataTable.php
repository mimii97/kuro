...
class InfosDataTable extends DataTable
{

    public function dataTable(DataTables $dataTables, $query)
    {
        return datatables($query)
            ->addColumn('actions', 'admin.infos.buttons.actions')

            ->addColumn('logo', '{!! view("admin.show_image",["image"=>$logo])->render() !!}')

			->addColumn('url', function ($info) {
				return "<a href=".$info->url." target=_blank>".$info->url."</a>";	
			})
   		...
   		->addColumn('created_at', '{{ date("Y-m-d H:i:s",strtotime($created_at)) }}')   		->addColumn('updated_at', '{{ date("Y-m-d H:i:s",strtotime($updated_at)) }}')            ->addColumn('checkbox', '<div  class="icheck-danger">
            ...
            ->rawColumns(['checkbox','actions',"logo", 'url']);
    }

	public function query()
    {
        return Info::query()->select("infos.*");

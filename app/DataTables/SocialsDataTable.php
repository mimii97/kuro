...
class SocialsDataTable extends DataTable
{
    ...
    public function dataTable(DataTables $dataTables, $query)
    {
        return datatables($query)
			...
			->addColumn('file', function ($social) {
				if($social->file)
					return "<a href=".url("/storage/".$social->file)." target=_blank> <i class='fa fa-download fa-2x'> </i> </a>";	
   		...
   		->addColumn('created_at', '{{ date("Y-m-d H:i:s",strtotime($created_at)) }}')   		->addColumn('updated_at', '{{ date("Y-m-d H:i:s",strtotime($updated_at)) }}')            ->addColumn('checkbox', '<div  class="icheck-danger">
            ...
            ->rawColumns(['checkbox','actions',"logo","file",'link']);
    }
  

	public function query()
    {
        return Social::query()->select("socials.*");
